...
A real-time crypto market screener built with Angular 17+. It streams live candlestick data directly from Binance WebSocket APIs, lets authenticated users manage a personal watchlist of favorites and a holdings portfolio with transaction history, and renders interactive OHLCV charts in the browser — no server-side data proxy needed for market data.
...
- **Holdings portfolio** — track crypto positions with weighted average cost basis, automatically recalculated on every transaction change
- **Transaction history** — log BUY/SELL transactions per holding; holdings are auto-deleted when all transactions are removed
...
This spins up MySQL 8 on port `3307` and runs `init.sql` to create all tables.
...
**Inspect the database** — connect via the MySQL CLI inside the running container:
...
GET    /api/holdings/transactions/:holdingId
...
## Holdings & transaction logic

Holdings and their transactions are managed entirely within `HoldingsService`. `TransactionService` exists as a thin delegation layer and routes calls through to `HoldingsService`.

`recalculateHolding` is called automatically after every `addTransaction` and `deleteTransaction`. It walks all transactions for a holding in chronological order and applies a weighted average cost algorithm:

- **BUY** — adds to `totalQty` and accumulates `totalCost`
- **SELL** — subtracts from `totalQty` and reduces `totalCost` proportionally
- If `totalQty` reaches zero after recalculation, the holding record is deleted automatically
- `avgPrice` is stored to 8 decimal places; intermediate cost ratio calculations use 18 decimal places to avoid rounding drift
